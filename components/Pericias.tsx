...
const Pericias = ({ pericias, isLoading, error }: PericiasProps): JSX.Element => {
  const [periciasList, setPericiasList] = useState<JudicialProcess[]>(pericias);
  const [selectedPericia, setSelectedPericia] = useState<JudicialProcess | null>(null);

  useEffect(() => {
    setPericiasList(pericias);
  }, [pericias]);

  const handleOpenModal = (pericia: JudicialProcess) => {
  ...
  const handleSavePericia = (updatedPericia: JudicialProcess) => {
  ...
  const renderTableContent = () => {
    ...
    return periciasList.map((pericia) => (
      <tr key={pericia.id} className="hover:bg-gray-800/60 transition-colors duration-200">
        ...
        <td className="p-4 text-brand-light text-sm">{pericia.defendant}</td>
        <td className="p-4">
          <span className={`px-3 py-1 text-xs font-bold rounded-full ${getStatusClass(pericia.status)}`}>
            {pericia.status}
          </span>
        </td>
        <td className="p-4 text-brand-gray text-sm">{new Date(pericia.startDate).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</td>

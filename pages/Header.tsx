...
interface HeaderProps {
  currentPage: 'home' | 'pericias';
  onNavigateHome: () => void;
  onNavigatePericias: () => void;
}

const Header: React.FC<HeaderProps> = ({ currentPage, onNavigateHome, onNavigatePericias }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-brand-dark shadow-lg' : 'bg-transparent'}`}>
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="#" onClick={(e) => { e.preventDefault(); onNavigateHome(); }} className="flex items-center space-x-3 cursor-pointer">
        ...
        <button 
          onClick={(e) => { e.preventDefault(); onNavigatePericias(); }}

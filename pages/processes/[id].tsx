    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 transition-opacity duration-300" aria-modal="true" role="dialog">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
                <div className="flex justify-between items-center p-5 border-b sticky top-0 bg-red-500 rounded-t-lg">
...
const FeesChargedModal: React.FC<{
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Gerenciar Propostas de Honorários">
...
const FeesReceivedModal: React.FC<{
...
}> = ({ isOpen, onClose, payments, onUpdate }) => {
    const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
    const [amount, setAmount] = useState('');
        ...
        if (date && amount && source) {
            if(editingPayment) {
                 const updatedPayments = payments.map(p => 
                    p.id === editingPayment.id ? { ...p, date, amount: parseFloat(amount), source } : p
                );
                onUpdate(updatedPayments);
            } else {
                const newPayment: Payment = { id: new Date().getTime().toString(), date, amount: parseFloat(amount), source };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Gerenciar Recebimentos de Honorários">
             <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 pb-6 border-b">
                <div>
                 ...
                 <div className="md:col-span-2">
                    <label htmlFor="paymentSource" className="block text-sm font-medium text-gray-700">Origem / Descrição</label>
                ...
                <div>
                    <label htmlFor="paymentAmount" className="block text-sm font-medium text-gray-700">Valor (R$)</label>
                ...
                <div className="md:col-span-4 self-end flex space-x-2">
                     <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md transition duration-300">{editingPayment ? 'Atualizar' : 'Adicionar'}</button>
